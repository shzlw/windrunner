...
windrunner work-items list PROJECT_ID --parent-id PROJECT_ROOT --json
windrunner work-items list PROJECT_ID --parent-id PARENT_WORK_ITEM_ID --json
...
For `work-items list`, omit `--parent-id` to include all hierarchy levels. Use
`PROJECT_ROOT` to return only top-level items, or provide a work-item ID to
return its direct children.
