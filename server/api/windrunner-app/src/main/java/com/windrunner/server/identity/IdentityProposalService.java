...
public class IdentityProposalService {
    ...
    public List<Created> create(ToolExecutionContext context, String sourceMessageId, Kind kind, List<Draft> drafts) {
        ...
        if (kind == null || drafts == null || drafts.isEmpty() || drafts.size() > 25)
            throw bad("Provide between 1 and 25 changes");
        ...
        for (IdentityProposal proposal : page.stream().limit(limit).toList()) {
            ...
            try {
                ...
                for (int index = 0; index < proposalChanges.size(); index++)
                    authorize(Kind.valueOf(proposalChanges.get(index).getEntityType()), drafts.get(index), actor);
    ...
    public View decide(String sessionId, String id, String decision, AppUser requestedActor) {
        ...
        for (int index = 0; index < proposalChanges.size(); index++)
            authorize(Kind.valueOf(proposalChanges.get(index).getEntityType()), drafts.get(index), actor);
        if (proposals.claimForDecision(id, sessionId, actor.getId()) != 1)
            throw error(HttpStatus.CONFLICT, "Proposal already decided");
        if ("ACCEPT".equals(decision)) {
            ...
            for (int index = 0; index < proposalChanges.size(); index++) {
                ...
                if (!beforeMatches || !afterMatches)
                    throw error(HttpStatus.CONFLICT, "This target changed after the proposal was created. Ask AI to review it again.");
            ...
            for (IdentityProposalChange change : proposalChanges)
                if (changes.decide(change.getId(), id, "APPLIED", null) != 1)
                    throw error(HttpStatus.CONFLICT, "Proposal change was already decided");
            proposal.setStatus("APPLIED");
        } else {
            for (IdentityProposalChange change : proposalChanges)
                if (changes.decide(change.getId(), id, "REJECTED", null) != 1)
                    throw error(HttpStatus.CONFLICT, "Proposal change was already decided");
            proposal.setStatus("REJECTED");
        }
        if (proposals.decide(id, sessionId, actor.getId(), proposal.getStatus()) != 1)
            throw error(HttpStatus.CONFLICT, "Proposal already decided");
    ...
    private View decideLegacy(IdentityProposal proposal, String sessionId, String decision, AppUser actor) {
        Kind kind = Kind.valueOf(proposal.getKind());
        Draft draft = readDraft(proposal);
        authorize(kind, draft, actor);
        if (proposals.claimForDecision(proposal.getId(), sessionId, actor.getId()) != 1)
            throw error(HttpStatus.CONFLICT, "Proposal already decided");
        if ("ACCEPT".equals(decision)) {
            actor = auth.requireActiveActor(actor);
            authorize(kind, draft, actor);
            Map<String, String> expected = readMap(proposal.getBeforeJson());
            Prepared current = prepare(kind, draft, actor);
            if (!current.before().equals(expected) || !current.after().equals(readMap(proposal.getAfterJson())))
                throw error(HttpStatus.CONFLICT, "This target changed after the proposal was created. Ask AI to review it again.");
            apply(kind, draft, expected, current.after(), actor);
            proposal.setStatus("APPLIED");
        } else proposal.setStatus("REJECTED");
        if (proposals.decide(proposal.getId(), sessionId, actor.getId(), proposal.getStatus()) != 1)
            throw error(HttpStatus.CONFLICT, "Proposal already decided");
    ...
    private AppUser requireSession(String sessionId, AppUser requestedActor) {
        ...
        if (sessionId == null || sessions.findByIdAndUserId(sessionId, actor.getId()).isEmpty())
            throw error(HttpStatus.NOT_FOUND, "Chat session not found");
        return actor;
    }

    private AppUser activeActor(ToolExecutionContext context) {
        return authorization.requireActor(context);
    }
    ...
    private void authorizeLegacy(Kind kind, Draft draft, AppUser actor) {
        if (kind == Kind.PROJECT_MEMBERSHIP)
            ...
            if (kind == Kind.USER_PROFILE || kind == Kind.USER_ACCESS) {
                ...
                if (kind == Kind.USER_ACCESS && draft.fields() != null && draft.fields().containsKey("globalRole") && !AppRoles.isSuperAdmin(actor.getGlobalRole()))
    ...
    private Prepared prepareTeam(Draft d, AppUser actor) {
        ...
        Map<String, String> fields = fields(d, Set.of("name", "description"));
        Map<String, String> before = new LinkedHashMap<>();
        Team team = new Team();
        String id = null;
        if ("UPDATE".equals(d.action())) {
            id = required(d.teamId(), "Team ID");
        ...
        if (fields.containsKey("name")) team.setName(required(fields.get("name"), "Name"));
        if (fields.containsKey("description")) team.setDescription(fields.get("description"));
        teams.validateTeamChange(id, team, d.ownerUserIds(), actor);
        ...
        if (id == null) {
            ...
            userRepository.findAllById(owners).forEach(user -> ownerNames.add(display(user)));
    ...
    private Prepared prepareTeamMembership(Draft d) {
        String teamId = required(d.teamId(), "Team ID"), userId = required(d.userId(), "User ID");
        Team team = teams.getTeam(teamId);
        AppUser user = memberUser(userId);
        var existing = teamMembers.findByTeamIdAndUserId(teamId, userId).orElse(null);
        String oldRole = existing == null ? null : existing.getRole();
        membershipAction(d.action(), oldRole);
        String newRole = "REMOVE".equals(d.action()) ? null : teamRole(d.role());
    ...
    private Prepared prepareProjectMembership(Draft d) {
        String projectId = required(d.projectId(), "Project ID");
        var project = projects.findById(projectId).orElseThrow(() -> error(HttpStatus.NOT_FOUND, "Project not found"));
        String subjectType = required(d.subjectType(), "Subject type");
        if (IdentityProposalSupport.present(d.userId()) == IdentityProposalSupport.present(d.teamId()))
            throw bad("Supply exactly one userId or teamId");
        String oldRole;
        ...
        if ("USER".equals(subjectType)) {
            String id = required(d.userId(), "User ID");
            before.put("userId", id);
            before.put("user", display(memberUser(id)));
            oldRole = projectMembers.findByProjectIdAndUserId(projectId, id).map(m -> m.getRole()).orElse(null);
        } else if ("TEAM".equals(subjectType)) {
            String id = required(d.teamId(), "Team ID");
            before.put("teamId", id);
            before.put("team", teams.getTeam(id).getName());
            oldRole = projectTeams.findByProjectIdAndTeamId(projectId, id).map(m -> m.getRole()).orElse(null);
        } else throw bad("Subject type must be USER or TEAM");
        membershipAction(d.action(), oldRole);
        String newRole = "REMOVE".equals(d.action()) ? null : projectRole(d.role());
        ...
        if ("USER".equals(subjectType))
            projectMembers.findByProjectIdAndUserId(projectId, required(d.userId(), "User ID")).ifPresent(member -> putRevision(before, "membershipUpdatedAt", member.getUpdatedAt()));
        else
            projectTeams.findByProjectIdAndTeamId(projectId, required(d.teamId(), "Team ID")).ifPresent(member -> putRevision(before, "membershipUpdatedAt", member.getUpdatedAt()));
        Map<String, String> after = new LinkedHashMap<>(before);
        after.put("role", newRole);
        return new Prepared(before, after);
    }
    ...
    private Prepared prepareUser(Kind kind, Draft d, AppUser actor) {
        if (!"UPDATE".equals(d.action())) throw bad("User proposals support UPDATE only");
        Map<String, String> requested = fields(d, kind == Kind.USER_PROFILE ? PROFILE_FIELDS : Set.of("status", "globalRole"));
        UserResponse current = users.getUser(d.userId(), actor);
        ...
        UpdateUserRequest request = userRequest(d, after);
        users.validateUpdate(d.userId(), request, actor);
        ...
        switch (kind) {
            case TEAM -> {
                if ("ADD".equals(d.action()))
                    teams.createTeam(new CreateTeamRequest(after.get("name"), after.get("description"), d.ownerUserIds()), actor);
                else {
                    Team team = new Team();
                    team.setName(after.get("name"));
                    team.setDescription(after.get("description"));
                    teams.updateTeamIfUnchanged(d.teamId(), team, timestamp(expected.get("updatedAt")), actor);
                }
            }
            case TEAM_MEMBERSHIP ->
                    teams.applyMembershipOptimistic(d.teamId(), d.userId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
            case PROJECT_MEMBERSHIP -> {
                if ("USER".equals(d.subjectType()))
                    memberships.applyUserOptimistic(d.projectId(), d.userId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
                else
                    memberships.applyTeamOptimistic(d.projectId(), d.teamId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
            }
            case USER_PROFILE, USER_ACCESS ->
                    users.updateUserIfUnchanged(d.userId(), userRequest(d, after), timestamp(expected.get("updatedAt")), actor);
        }
    }

    private Map<String, String> targetRef(Draft d, Prepared prepared) {
    ...
    private Map<String, String> revisions(Map<String, String> values) {
    ...
    private UpdateUserRequest userRequest(Draft d, Map<String, String> values) {
    ...
    private Map<String, String> fields(Draft d, Set<String> allowed) {
    ...
    private AppUser memberUser(String id) {
        AppUser user = userRepository.findById(id).orElseThrow(() -> error(HttpStatus.NOT_FOUND, "User not found"));
        if (AppRoles.isSuperAdmin(user.getGlobalRole())) throw bad("Super admin users cannot be members");
        return user;
    }

    private void membershipAction(String action, String oldRole) {
        ...
        if (!"ADD".equals(action) && oldRole == null) throw error(HttpStatus.NOT_FOUND, "Membership not found");
    }

    private String teamRole(String role) {
        try {
    ...
    private String projectRole(String role) {
        try {
    ...
    private String display(AppUser user) {
        return user.getDisplayName() == null ? user.getUsername() : user.getDisplayName();
    }

    private Map<String, String> identity(String... pairs) {
        Map<String, String> result = new LinkedHashMap<>();
        for (int i = 0; i < pairs.length; i += 2) result.put(pairs[i], pairs[i + 1]);
        return result;
    }

    private Draft readDraft(IdentityProposal proposal) {
        return JsonUtils.fromJson(proposal.getDraftJson(), Draft.class);
    }

    private Draft readDraft(IdentityProposalChange change) {
        return JsonUtils.fromJson(change.getPayload(), Draft.class);
    }
    ...
    private Map<String, String> readMapOrEmpty(String json) {
        return json == null ? new LinkedHashMap<>() : readMap(json);
    }
    ...
    private View view(IdentityProposal proposal, List<IdentityProposalChange> proposalChanges) {
        List<ChangeView> views = proposalChanges.stream().map(change -> new ChangeView(change.getId(), Kind.valueOf(change.getEntityType()), action(change.getOperation()), change.getStatus(), readMapOrEmpty(change.getBeforeSnapshot()), readMapOrEmpty(change.getAfterSnapshot()))).toList();
        ChangeView first = views.getFirst();
        return new View(proposal.getId(), proposal.getSourceMessageId(), proposal.getWorkflowType(), first.kind(), proposalChanges.size() == 1 ? first.action() : "BATCH", proposal.getStatus(), first.before(), first.after(), views);
    }

    private View legacyView(IdentityProposal proposal) {
        Draft draft = readDraft(proposal);
        ...
        return new View(proposal.getId(), proposal.getSourceMessageId(), proposal.getWorkflowType(), change.kind(), change.action(), change.status(), change.before(), change.after(), List.of(change));
    }
