...
public class ProposalService {
     ...
     */
    private final ProposalWorkflow<Draft> proposalWorkflow;
    ...
    public List<Created> create(ToolExecutionContext context, String sourceMessageId, Kind kind, List<Draft> drafts) {
        ...
        for (Draft draft : drafts) {
            ...
            if (kind == Kind.PROJECT_MEMBERSHIP) toolAuthorizationService.requireProjectOwner(context, draft.projectId());
        ...
        if (proposalChangeRepository == null) {
            ...
            for (Pending item : pending) {
                ...
                proposalRepository.insert(id, workflowType(), context.chatSessionId(), sourceMessageId, actor.getId(), kind.name(),
        ...
        proposalRepository.insertParent(parentId, workflowType(),
        ...
        for (int index = 0; index < pending.size(); index++) {
            ...
            proposalChangeRepository.insert(changeId, parentId, index, kind.name(), operation(item.draft().action()),
    ...
    public Page list(String sessionId, AppUser requestedActor, int requestedLimit, int requestedOffset) {
        ...
        List<Proposal> page = proposalRepository.page(workflowType(), sessionId, actor.getId(), limit + 1, offset);
    ...
    public View decide(String sessionId, String id, String decision, AppUser requestedActor) {
        AppUser actor = requireSession(sessionId, requestedActor);
        Proposal proposal = proposalRepository.findForDecision(workflowType(), id, sessionId, actor.getId()).orElseThrow(() -> error(HttpStatus.NOT_FOUND, "Proposal not found"));
    ...
    private View decideLegacy(Proposal proposal, String sessionId, String decision, AppUser actor) {
        Kind kind = Kind.valueOf(proposal.getKind());
    ...
    private String workflowType() {
        return proposalWorkflow == null ? "IDENTITY" : proposalWorkflow.workflowType();
    }
    ...
    private Prepared prepareProjectMembership(Draft d) {
        ...
        var project = projectRepository.findById(projectId).orElseThrow(() -> error(HttpStatus.NOT_FOUND, "Project not found"));
        String subjectType = required(d.subjectType(), "Subject type");
        if (present(d.userId()) == present(d.teamId()))
        ...
        if ("USER".equals(subjectType))
            projectMemberRepository.findByProjectIdAndUserId(projectId, required(d.userId(), "User ID")).ifPresent(member -> putRevision(before, "membershipUpdatedAt", member.getUpdatedAt()));
        else
            projectTeamRepository.findByProjectIdAndTeamId(projectId, required(d.teamId(), "Team ID")).ifPresent(member -> putRevision(before, "membershipUpdatedAt", member.getUpdatedAt()));
        Map<String, String> after = new LinkedHashMap<>(before);
        after.put("role", newRole);
        return new Prepared(before, after);
    }
        ...
        switch (kind) {
            case TEAM -> {
                if ("ADD".equals(d.action()))
                    ...
                    teamService.updateTeamIfUnchanged(d.teamId(), team, timestamp(expected.get("updatedAt")), actor);
                }
            }
            case TEAM_MEMBERSHIP ->
                    teamService.applyMembershipOptimistic(d.teamId(), d.userId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
            case PROJECT_MEMBERSHIP -> {
                if ("USER".equals(d.subjectType()))
                    projectMembershipService.applyUserOptimistic(d.projectId(), d.userId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
                else
                    projectMembershipService.applyTeamOptimistic(d.projectId(), d.teamId(), after.get("role"), d.action(), timestamp(expected.get("updatedAt")), timestamp(expected.get("membershipUpdatedAt")), actor);
            }
            case USER_PROFILE, USER_ACCESS ->
                    userAdminService.updateUserIfUnchanged(d.userId(), userRequest(d, after), timestamp(expected.get("updatedAt")), actor);
        }
    }
    ...
    private Draft readDraft(Proposal proposal) {
        return JsonUtils.fromJson(proposal.getDraftJson(), Draft.class);
    }

    private Draft readDraft(ProposalChange change) {
    ...
    private View view(Proposal proposal, List<ProposalChange> proposalChanges) {
    ...
    private View legacyView(Proposal proposal) {
        Draft draft = readDraft(proposal);
