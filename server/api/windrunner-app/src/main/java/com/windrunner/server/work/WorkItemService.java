...
public class WorkItemService {
    ...
    @Transactional public WorkItem update(String projectId, String id, WorkItem item, List<WorkItemAssignee> requestedAssignees, String actorId) {
        WorkItem current = requireForUpdate(projectId, id); Map<String, Object> before = snapshot(current); item.setProjectId(projectId); item.setId(id); normalize(item, current);
    ...
    private WorkItem requireForUpdate(String projectId, String id) { return workItems.findByIdForUpdate(id).filter(x -> projectId.equals(x.getProjectId())).orElseThrow(() -> notFound("Work item not found")); }
