...
public class FetchMembershipTool implements Tool<FetchMembershipTool.Parameters> {
    ...
    public String description() {
        return "Look up exactly one direct membership before proposing changes. For a team, provide teamId and userId and leave projectId empty (admin required). For a project, provide a selected-context projectId and exactly one non-empty subject ID: userId for a user membership or teamId for a team membership; leave the other subject ID empty (project owner required). Returns exists and current role; this is not a report of all inherited access.";
    }
    ...
    public Object execute(Parameters p, ToolExecutionContext context) {
        ...
        if (projectId != null) {
            ...
            if ((userId == null) == (teamId == null))
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Supply exactly one userId or teamId");
            role = userId != null
            ...
            if (teamId == null || userId == null)
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "teamId and userId required");
