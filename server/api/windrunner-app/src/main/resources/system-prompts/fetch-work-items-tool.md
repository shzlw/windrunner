...
Use `parentWorkItemId` for an exact parent filter; use `PROJECT_ROOT` for top-level WorkItems and omit it when any parent is acceptable. Use `type` for an exact WorkItem type filter. The tool returns one page and includes `total`, `offset`, `limit`, and `hasMore`. For a complete list, request subsequent pages using `offset + limit`; do not treat the first page as the whole project.
</usage>
