...
Use it in batch for all members returned by `fetch_team_details` when profile details are needed.
Use `fetch_users` first when the user id is unknown and must be discovered by name, username, or email.
Only active users are returned. Do not infer details for users that are not returned.
</usage>
