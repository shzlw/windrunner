...
For an exact duplicate check, set `exact` to true, provide the full proposed Entry body as `query`, and provide its
`workItemId`. Exact mode compares the stored body after trimming and returns all matching Entries within the bounded
result limit. A non-exact search result is evidence for review, not proof that an Entry is or is not a duplicate.
</usage>
